...
$( document ).ready(function() {
    ...
    $.post("/course-detail.json", {courseId: id}, showIndividualCourseData);
});


function showIndividualCourseData(data) {

    $('#header').html("<h3>Course: " + data.course_name + "</h3>");
    $('#course-detail-data').html("<h4>Course Data</h4>");
    $("<tr><th>Course ID</th><th>Date Added</th><th>Distance (km)</th></tr></table>").appendTo('#course-detail-table');
    var row = $("<tr />");
    $("#course-detail-table").append(row); 
      row.append($("<td>" + data.course_id + "</td>"));
